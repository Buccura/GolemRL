...
{	public float player_speed = 5.0f; //Move speed
	...
	public float player_projectile_speed = 100.0f; //Base projectile speed
	...
	{	//Movement
        ...
        //Aiming
        Vector3 aim_point; //Where the mouse is pointing
        if (Camera.main != null)
        ...
        //---Below code might not be in world coordinates---//
		...
		//Update transforms
		transform.Translate(x_mov, 0.0f, z_mov); //Use velocity instead? Collision issues.
		//transform.Rotate(0.0f, turn_angle, 0.0f);

		//Firing
		if ( Input.GetMouseButtonDown(0) )
		{	GameObject shot = Instantiate(basic_bullet, transform.position + transform.forward, transform.rotation);
			shot.GetComponent<Rigidbody>().AddForce( new Vector3(x_mov, 0.0f, z_mov) + transform.forward*player_projectile_speed ); //Use velocity?
        }
	}
}
